    ...
    try {
        for (var i = 1; i <= app.project.numItems; i++) {
            ...
            if (item instanceof CompItem && item.name.indexOf(" ") !== -1) {
                out.push({ compIndex: i, label: item.name, status: "SPACE NAME" });
            }
        }
    } catch (e) {
            ...
            for (var j = 1; j <= comp.numLayers; j++) {
                ...
                if (fxGroup) {
                    for (var k = 1; k <= fxGroup.numProperties; k++) {
                        ...
                        var fxName = fx.name;
                        var isMissing = (
                            (fxName.indexOf("[") > -1 && fxName.indexOf("]") > -1) ||
                            fxName.toLowerCase().indexOf("error") > -1 ||
                            fxName.toLowerCase().indexOf("missing") > -1
                        );
                        if (isMissing) {
